...
# IT Service Desk Deployment Script - Working Version
# Run with: bash working_deploy.sh
...
# Fix PostgreSQL authentication for local connections
echo "Configuring PostgreSQL authentication..."
PG_VERSION=$(ls /etc/postgresql/ | head -1)
sudo sed -i.bak 's/local.*all.*all.*peer/local   all             all                                     md5/' /etc/postgresql/$PG_VERSION/main/pg_hba.conf
sudo sed -i 's/local.*all.*all.*scram-sha-256/local   all             all                                     md5/' /etc/postgresql/$PG_VERSION/main/pg_hba.conf
...
CREATE USER servicedesk_user WITH ENCRYPTED PASSWORD '$DB_PASSWORD';
...
export DATABASE_URL="postgresql://servicedesk_user:$DB_PASSWORD@localhost:5432/servicedesk"
npm run db:push
...
echo "  Manager: mike.wilson / password123"
echo ""
