...
# Clean Installation Script for IT Service Desk on Ubuntu (Fixed)
# Removes all existing files and performs fresh installation

set -e

echo "=== IT Service Desk - Clean Installation (Fixed) ==="
echo "This script will remove all existing files and perform a fresh installation"
echo ""

# Check if running as root
if [[ $EUID -ne 0 ]]; then
   echo "Please run this script with sudo privileges: sudo bash clean-install-fixed.sh"
   exit 1
fi
...
# Change to postgres home directory for database operations
cd /var/lib/postgresql

# Clean database
sudo -u postgres psql -c "DROP DATABASE IF EXISTS servicedesk;" 2>/dev/null || true
sudo -u postgres psql -c "DROP USER IF EXISTS servicedesk;" 2>/dev/null || true
...
# Create simple startup script for the application
echo "Step 14: Creating application startup..."
cat > /var/www/itservicedesk/start-app.sh << 'EOF'
#!/bin/bash
cd /var/www/itservicedesk
...
EOF
chmod +x /var/www/itservicedesk/start-app.sh

# Create systemd service
echo "Step 15: Creating systemd service..."
...
Environment=DATABASE_URL=postgresql://servicedesk:SecurePass123@localhost:5432/servicedesk
...
echo "✓ Application installed and running via systemd"
...
echo "   Stop:        sudo systemctl stop itservicedesk"
echo ""
...
echo "⚡ Application: Node.js systemd service on port 3000"