...
# Create clean working server
cat > working-server.cjs << 'SIMPLE_EOF'
...
# Create PM2 config
cat > working-server.config.cjs << 'CONFIG_EOF'
module.exports = {
  apps: [{
    name: 'servicedesk',
...
pm2 start working-server.config.cjs
pm2 save
sleep 20
...
CREATE_PRODUCT=$(curl -s -b /tmp/cookies.txt -X POST http://localhost:5000/api/products -H "Content-Type: application/json" -d '{"name":"Olympus Production","description":"Final working test","category":"software"}')
echo "$CREATE_PRODUCT"
