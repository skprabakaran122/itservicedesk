...
cat << 'WORKING_EOF'
# Create a complete working production server:

cd /var/www/itservicedesk

echo "=== CREATING WORKING PRODUCTION SERVER ==="
cat > working-production.cjs << 'SERVER_EOF'
...
app.get('/health', (req, res) => {
    res.json({ 
        ...
        environment: 'Working Production Server'
    });
});
...
cat > working-production.config.cjs << 'CONFIG_EOF'
module.exports = {
  apps: [{
    name: 'servicedesk',
    script: 'working-production.cjs',
    instances: 1,
    autorestart: true,
    max_restarts: 5,
    restart_delay: 3000,
...
echo ""
echo "=== DEPLOYING WORKING SERVER ==="
pm2 delete servicedesk 2>/dev/null
pm2 start working-production.config.cjs
pm2 save

sleep 30

echo ""
echo "=== TESTING WORKING SERVER ==="
...
CREATE_PRODUCT=$(curl -s -b /tmp/cookies.txt -X POST http://localhost:5000/api/products \
  -H "Content-Type: application/json" \
  -d '{"name":"Working Server Product","description":"Testing working production server","category":"software"}')
echo "$CREATE_PRODUCT"
...
if echo "$JOHN_AUTH" | grep -q '"user"' && echo "$CREATE_PRODUCT" | grep -q '"name"'; then
    echo "SUCCESS: Working production server deployed!"
    echo ""
    echo "✓ Server running without syntax errors"
