...
(async () => {
  try {
    ...
    console.log('[Auth] bcrypt loaded successfully via import');
  } catch (error) {
    console.log('[Auth] bcrypt failed to load:', error.message);
  }
})();

const PgStore = connectPgSimple(session);
const MemoryStoreSession = PgStore;
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use(session({
    ...
    cookie: {
      ...
      sameSite: 'lax' // Allow cross-site requests for Replit preview
    }
  }));
  ...
  app.post("/api/auth/forgot-password", async (req, res) => {
    try {
      ...
      await storage.setPasswordResetToken(user.email, resetToken, expiry);
      ...
      const emailHtml = `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p>You requested a password reset for your Calpion IT Service Desk account.</p>
      ...
      await emailService.sendEmail(user.email, "Password Reset - Calpion IT Service Desk", emailHtml);
  ...
  app.post("/api/tickets/:id/request-approval", async (req, res) => {
    try {
      ...
      // TEMP DEBUG - Remove this later
      console.log("=== APPROVAL DEBUG START ===");
      console.log("selectedManager:", selectedManager);
      console.log("comments:", comments);
      
      // Update ticket to pending approval status with selected manager and token
      const approvalComments = comments ? `Approval request sent to ${selectedManager.name} (${selectedManager.email}). Agent comments: ${comments}` : `Approval request sent to ${selectedManager.name} (${selectedManager.email})`;

      console.log("approvalComments FINAL:", approvalComments);
      ...
      const updatedTicket = await storage.updateTicketWithHistory(id, {
        ...
        approverEmail: selectedManager.email // Store manager email for history
  ...
  app.get("/approval/tickets/:id/approve/:token", async (req, res) => {
    try {
      ...
      }, ticket.requesterId, `Ticket approved via email by ${ticket.approvedBy}${ticket.approverEmail ? ` (${ticket.approverEmail})` : ""}`);
  ...
  app.get("/approval/tickets/:id/reject/:token", async (req, res) => {
    try {
      ...
      const updatedTicket = await storage.updateTicketWithHistory(id, {
        ...
        approvedAt: new Date(),
        approvalToken: null // Clear token after use
      }, 0, `Ticket rejected via email by ${ticket.approvedBy}${ticket.approverEmail ? ` (${ticket.approverEmail})` : ""}`);
  ...
  app.get("/api/products", async (req, res) => {
    try {
      ...
      if (!req.session?.user) return res.json(allProducts);
      const user = req.session.user;
      // Agents and admins can see all products to create tickets
