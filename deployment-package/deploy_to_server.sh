...
if [ -n "$GIT_REPO" ]; then
    ...
    if [ ! -f "package.json" ]; then
        echo "ERROR: package.json not found in /var/www/servicedesk"
        echo "Please copy your application files to this directory first."
        echo ""
        echo "You can copy files using one of these methods:"
        echo "1. scp -r /path/to/your/app/* user@server:/var/www/servicedesk/"
        echo "2. rsync -av /path/to/your/app/ user@server:/var/www/servicedesk/"
        echo "3. Manual upload via FTP/SFTP"
        echo ""
        read -p "Press Enter after copying files to continue..."
        
        # Check again after user confirmation
        if [ ! -f "package.json" ]; then
            echo "ERROR: package.json still not found. Deployment cannot continue."
            exit 1
        fi
    fi
fi
