...
echo "=== SYSTEMD SERVICE DEPLOYMENT FROM GITHUB ==="
...
app.get('/health', async (req, res) => {
    try {
        ...
        res.json({ 
            ...
            message: 'Production server - Systemd service deployment',
            database: {
                connected: true,
...
// Serve static files or basic API info
...
if (fs.existsSync(staticPath)) {
    ...
    app.get('*', (req, res) => {
        res.status(200).json({ 
            ...
            deployment: 'GitHub to Systemd Service'
        });
    });
}
...
app.listen(PORT, '0.0.0.0', () => {
    console.log(`[Server] Systemd service running on port ${PORT}`);
    console.log('[Server] Database: PostgreSQL servicedesk@localhost:5432/servicedesk');
    console.log('[Server] Process managed by systemd');
});
SERVER_EOF
fi
...
if echo "$HEALTH" | grep -q '"status":"OK"'; then
    echo "✓ Systemd service running successfully"
    ...
    CHANGE_CREATE=$(curl -s -b /tmp/test_cookies.txt -X POST http://localhost:5000/api/changes -H "Content-Type: application/json" -d '{"title":"Systemd Test","description":"Testing systemd service deployment","reason":"Validation of systemd deployment"}')
...
echo "✅ Service: $SERVICE_NAME managed by systemd"
...
echo ""
echo "PM2 module errors resolved - using native systemd service!"